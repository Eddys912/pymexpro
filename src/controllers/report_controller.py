...
class ProductionReportController:
    def get_all_reports(self):
        try:
            ...
            return reports if reports else {"message": "No se encontraron reportes de producción."}
        except Exception as e:
            print(f"Error al obtener reportes: {e}")
            return {"message": "Error al obtener reportes."}

    def create_report(self, manager_id, product_id, machine_id, produced_quantity, defective_quantity, production_date, comments):
        try:
            if not all([manager_id, product_id, machine_id, produced_quantity, production_date]):
                return {"message": "Todos los campos obligatorios no pueden estar vacíos."}
